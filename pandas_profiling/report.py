...
def to_html(sample,imp_features, stats_object):
    ...
    if len(stats_object['correlations']['pearson']) > 0:
        ...
        # Below line is added by Manmohan to include correlation matrix
        #corr_matrix = correlation_matrix(stats_object['correlations']['pearson'], 'Pearson')
    ...
    #Added by Manmohan
    imp_features_html = templates.template('imp_features').render(imp_feature_table_html=imp_features.to_html(classes="sample"))
    render_htmls['imp_features_html'] = imp_features_html
